...
export default function Admin() {
  ...
  // Helper: Merge Realtime presence with DB active visitors for zero-lag 100% reliable tracking
  const mergeVisitors = (realtimeList = [], dbList = []) => {
    ...
    for (const v of dbList) {
      if (v.visitor_id) {
        ...
        if (now - seenTime <= ACTIVE_CUTOFF_MS) {
          map.set(v.visitor_id, v);
        }
      }
    }

    // 2. Realtime WebSocket presence overlays DB (takes priority for active route & live connection)
    for (const v of realtimeList) {
      if (v.visitor_id) {
        ...
        map.set(v.visitor_id, {
          ...existing,
          ...v,
          online_at: existing?.online_at || v.online_at,
          last_seen_at: v.last_seen_at || existing?.last_seen_at || new Date().toISOString(),
        });
      }
    }
  ...
  // Real-Time Live Presence Monitor (Supabase WebSocket Presence + 10s DB Fallback Poll)
  useEffect(() => {
    if (!isAdmin) return;
      ...
      <div style={{ flexGrow: 1, padding: "20px 24px", position: "relative", zIndex: 5 }}>
        ...
        {isFlagsTableMissing && (
          <div className="admin-alert success" style={{ background: "rgba(251, 191, 36, 0.15)", border: "1px solid rgba(251, 191, 36, 0.3)", color: "#fef08a", fontSize: "12.5px", padding: "12px 18px", marginBottom: "18px", borderRadius: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button
              onClick={() => {
                const sql = "create table public.question_flags (\n  id bigint generated by default as identity primary key,\n  question_id bigint references public.questions(id) on delete cascade,\n  flagged_by text,\n  flag_type text default 'not_required',\n  created_at timestamp with time zone default now()\n);";
                if (navigator.clipboard) {
                  navigator.clipboard.writeText(sql);
                  setSuccess("SQL snippet copied to clipboard! Paste it into Supabase SQL editor.");
                } else {
                  alert("Run this SQL in your Supabase console:\n\n" + sql);
                }
              }}
              style={{ background: "rgba(251, 191, 36, 0.2)", border: "1px solid #fbbf24", color: "#fbbf24", cursor: "pointer", fontWeight: 700, fontSize: "12px", padding: "4px 10px", borderRadius: "6px" }}
            >
              📋 Copy SQL Snippet
            </button>
          </div>
        ...
        {isAnalyticsTableMissing && (
          <div className="admin-alert success" style={{ background: "rgba(56, 189, 248, 0.15)", border: "1px solid rgba(56, 189, 248, 0.35)", color: "#bae6fd", fontSize: "12.5px", padding: "12px 18px", marginBottom: "18px", borderRadius: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button
              ...
              style={{ background: "rgba(56, 189, 248, 0.2)", border: "1px solid #38bdf8", color: "#38bdf8", cursor: "pointer", fontWeight: 700, fontSize: "12px", padding: "4px 10px", borderRadius: "6px" }}
            >
              📋 Copy SQL Snippet
            </button>
          </div>
